...
| GUI Process Migration System Product Requirements Document v0.1  |  April 2026 |
| :---: |
...
* Native Wayland app support (deferred — protocol does not support reconnect)
...
| **Kernel** | \>= 5.15, with CONFIG\_CHECKPOINT\_RESTORE=y |
...
| **Privileges** | CAP\_SYS\_PTRACE \+ CAP\_SYS\_ADMIN, or run CRIU as root |
...
| Example CLI gpms run blender my\_scene.blendgpms run code /path/to/projectgpms run inkscape artwork.svg |
| :---- |
...
* Automatic: hang detector polls /proc/\<pid\>/status and send test X events; triggers migration after N seconds of unresponsiveness (configurable, default 30s)

* Manual: user runs gpms migrate \<session-id\> to trigger immediately

## **7.3 Migration Experience**

* User sees a non-blocking notification: "App is unresponsive. Migrating to remote host..."
